...
static ssize_t getDeviceInfor(struct file* file, struct kobject* kobj, struct bin_attribute* attr, char* buf, loff_t pos, size_t size)
{
...
static ssize_t setCalibrate(struct file* file,struct kobject* kobj, struct bin_attribute* attr, char* buf, loff_t pos, size_t size)
{
	struct device* dev = container_of(kobj, struct device, kobj);
	sensor_device* sDev = dev_get_drvdata(dev);
	ssize_t siz = -EPERM;
...
static ssize_t getCalibrate(struct file* file, struct kobject* kobj, struct bin_attribute* attr, char* buf, loff_t pos, size_t size)
{
	struct device* dev = container_of(kobj, struct device, kobj);
	sensor_device* sDev = dev_get_drvdata(dev);
	ssize_t siz = -EPERM;
...
{
	if(!cdev || !cdev->dev || IS_ERR(cdev->dev)) return;
